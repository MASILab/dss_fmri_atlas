...
def sphere2cart(r, theta, phi):
    """Convert spherical coordinates (opposite convention as Dipy) to Cartesian coordinates"""
    x = r * np.sin(phi) * np.cos(theta)
    y = r * np.sin(phi) * np.sin(theta)
    z = r * np.cos(phi)
    return x, y, z

def reorient_to_ras(nifti):
    """Reorient a NIFTI to RAS orientation

    Parameters
    ----------
    nifti : Nibabel.Nifti1Image
        Nifti in any orientation

    Returns
    -------
    nifti_ras : Nibabel.Nifti1Image
        Nifti in RAS orientation
    """
    # Reorient to standard
    old_affine = nifti.affine
    axcodes = nib.aff2axcodes(old_affine)
    new_axcodes = ('R', 'A', 'S')
    old_ornt = nib.orientations.axcodes2ornt(axcodes)
    new_ornt = nib.orientations.axcodes2ornt(new_axcodes)
    transform_ornt = nib.orientations.ornt_transform(old_ornt, new_ornt)

    # Reorient the FA
    data = nib.orientations.apply_orientation(nifti.get_fdata(), transform_ornt)
    ras_affine = nib.orientations.inv_ornt_aff(transform_ornt, data.shape)
    affine = np.dot(old_affine, ras_affine)

    nifti_ras = nib.Nifti1Image(data, affine, nifti.header)
    return nifti_ras
